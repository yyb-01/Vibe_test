#include "console.hpp"
#include <iostream>

std::string label(Id value) {
    if (!value) return "0";
    return value.hi == 1 ? std::to_string(value.lo) : std::to_string(value.hi) + ':' + std::to_string(value.lo);
}
void show(const World& w) {
    for (const auto& [id, c] : w.containers) {
        const auto& s = c.state;
        std::cout << "Container " << label(id) << " rev=" << s.revision
                  << " mass=" << s.subtreeMassG << "g volume=" << s.usedVolumeMl << '/' << s.capacityMl
                  << "mL ownerItem=" << label(s.ownerItem) << '\n';
        for (const auto& [itemId, p] : w.placements) {
            if (p.container != id) continue;
            const auto& item = w.items.at(itemId);
            std::cout << "  item " << label(itemId) << " def=" << item.defId << " qty=" << item.quantity
                      << " grid=" << p.x << ',' << p.y << " rev=" << item.revision << '\n';
        }
    }
}
