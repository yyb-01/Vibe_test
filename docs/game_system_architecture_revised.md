...
작전 전체의 권위 경계는 한 프로세스의 OperationController에 있다. 개별 권위 상태의 실제 작성은 아래 단일 작성자가 맡고, OperationController는 기능 간 순서와 복합 거래를 조정한다.
...
| Damage, Health, Status | CombatController |
...
한 상태를 두 Controller가 직접 수정하지 않는다. Entity Node는 자신의 Health나 Status를 임의로 수정하지 않고 CombatController가 확정한 결과를 반영한다.

여러 상태를 함께 바꿔야 하면 OperationController가 거래를 조정한다. 단, OperationController는 규칙 구현자가 아니라 orchestration 경계다. BuildRules, InventoryRules, DamageRules 같은 기능 규칙을 복제하거나 직접 구현하지 않고, 각 기능 Controller의 공개 API를 호출해 순서와 원자적 Commit만 조정한다. 새로운 도메인 규칙을 OperationController의 조건문으로 추가하는 것은 금지한다.
...
### 4.2 Action의 범위와 공통 필드

Action은 플레이어 입력, UI 요청, Scripted Event처럼 기능 경계 밖에서 들어오는 변경 의도를 표현한다. PlaceBuildAction, TransferItemAction, AttackAction, RequestExtractionAction처럼 요청의 승인·거절과 중복 방지가 필요한 변경을 Action으로 만든다.

Controller 내부의 결정적 상태 전이까지 Action 객체로 만들지는 않는다. AI 의사결정 Tick, Status 만료, Reload 완료, Build Work 증가처럼 이미 소유 Controller의 Tick 안에서 발생한 전이는 해당 Controller와 Rules가 처리하고, 외부에 알려야 할 확정 결과만 Fact로 발행한다.
...
싱글플레이에서도 action_id를 유지한다. 버튼 중복 입력, 저장 복구 후 재실행을 같은 방식으로 방지한다. 향후 권위 프로세스 분리가 필요해져도 이 경계를 재사용할 수 있지만, G1에서 네트워크 프로토콜 수준의 전역 Action 로그를 만들지는 않는다.

### 4.3 ActionResult

외부 변경 Action API는 ActionResult를 반환한다. Controller 내부 Tick 전이는 ActionResult를 만들 필요가 없다.
...
둘 이상의 권위 상태를 함께 바꾸는 Action은 `Prepare → CommitPlan → Apply` 순서를 따른다.

- Prepare: 현재 상태를 읽어 검증하고 필요한 변경을 계산한다. 권위 상태를 수정하지 않는다.
- CommitPlan: 성공 시 적용할 최소 변경 집합을 순수 데이터로 고정한다.
- Apply: CommitPlan을 검증 없이 결정적으로 적용한다. Apply 단계에는 정상 게임 흐름에서 실패 가능한 작업을 넣지 않는다.

PlaceBuildAction의 예:

1. BuildRules가 Footprint와 Grid를 검증한다.
2. InventoryRules가 비용 Reservation 계획을 만든다.
3. BuildGrid가 Cell Reservation 계획을 만든다.
4. PathService가 계획 적용 후의 Grid에서 필수 경로를 확인한다.
5. BuildSite 생성에 필요한 Runtime 데이터를 계산한다.
6. 모든 Prepare가 성공하면 하나의 BuildCommitPlan으로 고정한다.
7. OperationController가 같은 Apply 경계에서 InventoryController와 BuildController의 apply 메서드를 정해진 순서로 호출한다. 각 Controller만 자신이 소유한 상태를 수정한다.
8. Apply 메서드는 이미 검증된 CommitPlan의 값만 반영하며 정상 흐름에서 실패하지 않아야 한다.
9. 하나라도 Prepare에 실패하면 어떤 권위 상태도 바꾸지 않는다.

Rollback을 정상 제어 흐름으로 사용하지 않는다. Apply 도중 예외가 발생할 수 있는 파일 I/O, Resource 로드, Scene 로드는 넣지 않는다. 필요한 Resource와 Scene 계약은 Boot Validator에서 검증하고 미리 로드한다.
...
G1의 결정성 보장은 Rule, Action 처리, Runtime State, 명명된 RNG Stream의 결과를 대상으로 한다. Godot Physics 충돌의 부동소수점 결과나 프레임 단위 위치를 플랫폼 간 bit-exact replay하는 것은 G1의 목표가 아니다.

동일한 Catalog, Seed, 동일한 규칙 입력과 Action Sequence를 두 번 실행하여 다음을 비교한다.
...
- Inventory
- RewardProposal
- 규칙이 소비한 RNG Stream 상태
- 생성된 게임 Fact 순서

VFX, Audio, Tween, 파티클, 카메라와 Physics의 프레임별 부동소수점 결과는 비교 대상이 아니다. 다만 Physics 후보가 규칙 결과에 들어오는 경계에서는 정렬과 ID 기반 tie-break를 사용해 가능한 한 안정적인 입력 순서를 만든다.
...
- 각 권위 상태에는 한 작성자만 있다.
- Health와 Status의 논리 작성자는 CombatController 하나다.
- OperationController는 기능 규칙을 구현하지 않고 기능 간 orchestration과 복합 Commit만 조정한다.
...
- 기능 경계 밖에서 들어오는 변경 의도는 공개 Action을 통한다.
- 소유 Controller 내부의 결정적 Tick 전이는 별도 Action 객체를 강제하지 않는다.
- 거절된 Action은 상태를 바꾸지 않는다.
- 복합 변경은 Prepare에서 모두 검증한 뒤 CommitPlan 하나로 Apply한다.
